...
function solution(model::JuMP.Model)::OrderedDict
    ...
    function net_power(g)
        return [
            OrderedDict("time period $(t)" => value(model[:net_power][g.name, t]) for t in 1:T)
        ]
    end
    function chr_power(g)
        return [
            OrderedDict("time period $(t)" => value(model[:chr_power][g.name, t]) for t in 1:T)
        ]
    end
    function dis_power(g)
        return [
            OrderedDict("time period $(t)" => value(model[:dis_power][g.name, t]) for t in 1:T)
        ]
    end
    function stored_energy(g)
        return [
            OrderedDict("time period $(t)" => value(model[:stored_energy][g.name, t]) for t in 1:T)
        ]
    end
    function average_hourly_cost(g)
        return [
            OrderedDict("time period $(t)" => value(model[:chr_power][g.name, t]) * 
    ...
    function average_hourly_revenue(g)
        return [
            OrderedDict("time period $(t)" => value(model[:dis_power][g.name, t]) * 
            sum(sc.price[t] * sc.probability for sc in model[:instance].prices) for t in 1:T)
        ]
    end
    ...
    function final_energy(g)
        return value(model[:stored_energy][g.name, T] + (model[:chr_power][g.name, T] * tm * g.efficiency) -
            (model[:dis_power][g.name, T] * tm / g.efficiency))
    end
    sol = OrderedDict()
    for g in instance.units
        sol[g.name] = OrderedDict(
            "Net discharging power (MW)" => net_power(g),
            ...
            "Average hourly costs (\$)" => average_hourly_cost(g), 
            "Average hourly revenues (\$)" => average_hourly_revenue(g),
            "Average total cost (\$)" => average_total_cost(g), 
            "Average total revenue (\$)" => average_total_revenue(g),
            "Average total profit (\$)" => average_total_profit(g),
        )
    end
    return sol
end
