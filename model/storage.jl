...
function _add_injection_withdrawal_vars!(model::JuMP.Model, g::Unit)::Nothing
    net_power = _init(model, :net_power)
    ...
    for t in 1:model[:instance].time
        net_power[g.name, t] = @variable(model)
...
function _add_operational_constraints!(model::JuMP.Model, g::Unit)::Nothing
    ...
    net_power_def=_init(model,:net_power_def)
    ...
    ini_energy[gn] = @constraint(
        model, 
        stored_energy[gn, 1] == g.initial_energy)
    
    for t in 1:T-1
        ener_stor[gn, t] = @constraint(
            model,
            stored_energy[gn, t+1] == stored_energy[gn, t] + (chr_power[gn, t] * tm * g.efficiency) - (dis_power[gn, t] * tm / g.efficiency)
            )
    end

    for t in 1:T
        net_power_def[gn, t] = @constraint(
            model,
            net_power[gn, t] == dis_power[gn, t] - chr_power[gn, t]
        )
        ...
        dis_power_max[gn, t] = @constraint(
            model,
            dis_power[gn, t] <= g.max_discharging_power
        )
        ...
        chr_power_max[gn, t] = @constraint(
            model,
            chr_power[gn, t] <= g.max_charging_power
        )
