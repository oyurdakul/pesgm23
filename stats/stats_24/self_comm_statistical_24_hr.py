...
error_matrix =error_matrix.iloc[:,0:49]
error_matrix = error_matrix.dropna(axis=0)
error_mat_nums = error_matrix.iloc[:,1:]

col_names_mat = list(error_matrix .columns)
...
error_matrix_df = pd.DataFrame(error_matrix,columns=col_names_mat)
...
error_matrix_df.to_csv("./results/error_time_series.csv")
...
plt.show
plt.savefig('./results/plots/prob_plot'+ str(z) + '.png')
